...
# -----------------------------------------------------------------------------
tab1, tab2 = st.tabs(["📄 Single Document Analysis", "📊 System Eval & Regression Hub"])
...
with tab1:
    ...
    st.caption("Upload a financial PDF report to analyze field-isolated evidence, extracted JSON, and runtime validation flags.")

    col_left, col_right = st.columns([1, 2])

    with col_left:
        uploaded_file = st.file_uploader("Upload PDF Document", type=["pdf"])
        doc_id_input = st.text_input("Document Identifier (doc_id)", value="doc_report_2024")
        namespace_input = st.text_input("Pinecone Namespace", value="analysis_demo")
        
        analyze_btn = st.button("Run Extraction Pipeline", type="primary", use_container_width=True)
    ...
    with col_right:
        if analyze_btn and uploaded_file is not None:
            ...
            with st.spinner("Extracting PDF text and generating chunks..."):
                ...
                chunks = ingestor.extract_and_chunk(pdf_path=tmp_pdf_path, doc_id=doc_id_input)
                st.toast(f"Extracted {len(chunks)} text chunks.", icon="📑")

            with st.spinner("Indexing vectors into Pinecone namespace..."):
                vstore.index_chunks(chunks=chunks, namespace=namespace_input)
                st.toast("Indexed chunks into Pinecone.", icon="🌲")

            with st.spinner("Running Field-Aware Retrieval, LLM Interpreter, and Runtime Validation..."):
                extraction_result, val_result, retries_used = pipeline.process_document(
                    doc_id=doc_id_input,
    ...
    if "latest_extraction" in st.session_state:
        ...
        if not val_res.is_valid:
            with st.expander("🚨 Validation Diagnostics & Failed Rules", expanded=True):
                for issue in val_res.issues:
                    st.warning(f"**Field:** `{issue.field_name}` | **Type:** {issue.issue_type}\n\n{issue.description}")
        ...
        for field_name, field_res in ext_res.extractions.items():
            with st.expander(f"Field: **{field_name}** — Confidence: `{field_res.confidence}`", expanded=True):
                c1, c2 = st.columns([1, 1])
                with c1:
                    st.markdown("**Extracted Value:**")
                    st.info(field_res.value if field_res.value else "*No value extracted*")
                
                with c2:
                    st.markdown("**Qualifiers & Context:**")
                    st.write(field_res.qualifiers if field_res.qualifiers else "*No explicit qualifiers detected*")

                st.markdown(f"**Cited Chunk IDs:** `{field_res.cited_chunk_ids}`")
...
with tab2:
    ...
    st.caption("Measure pipeline quality across the Golden Dataset and check for performance regressions.")
    ...
    with col_eval_1:
        ...
        st.write("Executes Retrieval Recall@K and Interpreter accuracy tests over ground-truth documents.")
        
        run_eval_btn = st.button("Execute Eval Harness", type="primary", use_container_width=True)
    ...
    with col_eval_2:
        if run_eval_btn:
            if not os.path.exists(golden_path):
                ...
                with st.spinner("Running Evaluation Harness over Golden Dataset..."):
                    harness = EvaluationHarness(pipeline=pipeline)
                    metrics = harness.run_eval(golden_dataset_path=golden_path, schema=active_schema)
    ...
    if "eval_metrics" in st.session_state:
        ...
        k1.metric("Retrieval Recall@K", f"{metrics.retrieval_recall_at_k * 100:.1f}%")
        k2.metric("Extraction Accuracy", f"{metrics.extraction_accuracy * 100:.1f}%")
        k3.metric("Nuance Score", f"{metrics.nuance_preservation_score * 100:.1f}%")
        k4.metric("Overall System Score", f"{metrics.overall_score * 100:.1f}%")

        st.markdown("### Regression Gate Status")
        if reg["status"] == "PASS":
            st.success(f"✅ **PASS**: Current Score ({reg['current_score']}) meets or exceeds Baseline ({reg['baseline_score']}). Delta: +{reg['diff']}")
        elif reg["status"] == "BASELINE_CREATED":
            st.info("ℹ️ Baseline initialized from this evaluation run.")
        else:
            st.error(f"❌ **REGRESSION DETECTED**: Current Score ({reg['current_score']}) fell below Baseline ({reg['baseline_score']}). Delta: {reg['diff']}")